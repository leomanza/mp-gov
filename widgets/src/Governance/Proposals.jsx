...
if (!state.lastProposalIdIsFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((lastProposalId) => State.update({ lastProposalId, lastProposalIdIsFetched: true })
    );
}
...
if (!state.proposalsAreFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((proposals) => { console.log('proposals', proposals); State.update({ proposals, proposalsAreFetched: true }) }
    );
}
...
if (!state.quorumIsFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((quorum) => State.update({ quorum: parseInt(quorum), quorumIsFetched: true })
    );
}
...
if (!state.thresholdIsFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((threshold) => State.update({ threshold: yoctoToNear(threshold), thresholdIsFetched: true })
    );
}
...
if (!state.votersAreFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((voters) => State.update({ voters: parseInt(voters), votersAreFetched: true })
    );
}
...
if (!state.totalHoldersIsFetched) {
    Near.asyncView(
        ...
        "final",
        false
    ).then((totalHolders) => { console.log('total holders', totalHolders); State.update({ totalHolders: parseInt(totalHolders), totalHoldersIsFetched: true }) }
    );
}
